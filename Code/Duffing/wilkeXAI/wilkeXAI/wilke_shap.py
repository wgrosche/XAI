...
class wilke_explainer():
    ...
    def eval_explainer(self):
        ...
        """
        first_run = True
        for i, __feature in enumerate(self.test_data.columns[:4]):
            arr = self.choose_data(i, __feature, len(self.test_data.columns[:4]))
...
class TrueModel():
    ...
    def predict(self, X):
        ...
        """
        X = pd.DataFrame(self.scaler.inverse_transform(X.loc[:,['x0','v0','t']]), columns=self.cols)
